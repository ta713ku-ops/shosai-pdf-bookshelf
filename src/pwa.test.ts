...
describe('service worker request boundaries', () => {
  ...
  it('uses a cached shell on network failure and deletes only its own obsolete caches', async () => {
    ...
    const storage = { open: vi.fn().mockResolvedValue(cache), keys: vi.fn().mockResolvedValue(['shosai-shell-v0', 'shosai-shell-v1', 'shosai-shell-v2', 'shosai-shell-v3', 'shosai-shell-v4', 'another-app']), delete: vi.fn().mockResolvedValue(true) }
    const claim = vi.fn()
