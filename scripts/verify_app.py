...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        shelf_cover = page.get_by_role("button", name="book-01を開く。1/3ページ")
        shelf_cover_image = shelf_cover.locator("img")
        shelf_cover_image.wait_for()
        cover_source = shelf_cover_image.get_attribute("src")
        if not cover_source or not shelf_cover_image.evaluate("element => element.complete && element.naturalWidth > 0"):
            raise AssertionError("The generated book cover was not visible before reading")
        shelf_cover.click()
