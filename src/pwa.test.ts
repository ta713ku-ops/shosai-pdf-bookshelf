...
describe('service worker request boundaries', () => {
  ...
  it('uses a cached shell on network failure and deletes only its own obsolete caches', async () => {
    ...
    const storage = { open: vi.fn().mockResolvedValue(cache), keys: vi.fn().mockResolvedValue(['shosai-shell-v0', 'shosai-shell-v1', 'shosai-shell-v2', 'shosai-shell-v3', 'shosai-shell-v4', 'shosai-shell-v5', 'another-app']), delete: vi.fn().mockResolvedValue(true) }
    const claim = vi.fn()
    ...
    expect(storage.delete).toHaveBeenCalledWith('shosai-shell-v3')
    expect(storage.delete).toHaveBeenCalledWith('shosai-shell-v4')
